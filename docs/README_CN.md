...
- **内存调试**：完成精确、模糊、联合、范围、临近搜索，支持搜索时间线恢复历史结果、手动写入撤回、结果筛选、批量修改，以及数值视图 / Hex 视图跳转。
...
- **内存搜索与批量调试**：支持精确、模糊、联合、范围、临近等多种搜索方式；支持搜索时间线快照与恢复、手动修改值撤回、大于 / 小于 / 区间筛选、固定值修改、递增修改、批量锁定、批量收藏、批量删除等高频操作。
